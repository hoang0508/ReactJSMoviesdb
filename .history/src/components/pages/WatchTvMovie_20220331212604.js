...
const WatchTvMovie = () => {
  ...
  const dataInfo =
    dataSeason.length > 0 && data.find((item) => item.season_number === query);
  console.log(
    "🚀 ~ file: WatchTvMovie.js ~ line 36 ~ WatchTvMovie ~ dataInfo",
    dataInfo
  );
  return (
    <>
      <div className="watchTV">
        <div className="container">
          ...
          {/* <div>
            {dataSeason.length > 0 &&
              dataSeason
                .filter((item) => Number(item.season_number) === query)
                .map((item) => <WatchTvMovie key={item.id} item={item} />)}
          </div> */}
        </div>
      </div>
    </>
